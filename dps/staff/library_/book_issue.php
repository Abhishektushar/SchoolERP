...
if(isset($_SESSION['post_assigned']))
   {
      if($_SESSION['post_assigned'] != "Librarian")
       header("location:../../");
    }      
?>
<?php
  if(!$staffdatabase->validateStudentID($_GET['id']))
...
<body>
      ...
      <div class="element">
              ...
              <div class="form-row">
                <div class="container">
                    <div class="form-group row">   
                      <?php $staffdatabase->getPreviousBookIssueInfo($_GET['id'],$_SESSION['post_assigned']);?>
                          </div><span class="col-md-1"></span>
                      <?php $staffdatabase->student_inf($_GET['id'])?>
                    </div>
                    <!-- if student has a pending fine -->
                    <?php if($val=$staffdatabase->checkpendingFINES(($_GET['id']))){?>
                    ...
                    <div class="container" align="center" id="bookform">
                        ...
                        <button type="button" name="cancel-request" onclick="change()" id="cancel-request" class="btn btn-primary">CANCEL</button>
                          ...
                          {
                            location.href="../library_/book_issue.php?id=<?php echo($_GET['id'])?>";
                          }                
            ...
            <div class="modal-dialog modal-sm">
              <div class="modal-content ">
                <div class="modal-header">
                  <h6 class="modal-title">Confirmation Require</h6>
          ...
          <div class="modal fade" id="payFine-confirmation" role="dialog">
            <div class="modal-dialog modal-sm">
              <div class="modal-content">
                <div class="modal-header">
                  <h6 class="modal-title">Remove Fine <?php echo $staffdatabase->checkpendingFINES(($_GET["id"]))?> /-</h6>
                  <button type="button" class="close" data-dismiss="modal">&times;</button>                
                </div>
                <div class="modal-body">
                  <form method="POST">
  ...
  <?php 
    if(isset($_POST["save-details"]))
    {
      ...
      $staffdatabase->addbookToStudent($sid,$bid,$return_date);
    }
    if(isset($_POST["complete-fine"]))
    {
      $sid=strip_tags($_GET["id"]);
      $staffdatabase->payAllfine($sid,"book_issue.php");
    }
  ?>