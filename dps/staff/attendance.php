...
?>
<?php
    if(!isset($_SESSION['class_teacher']))
...
<head>
    ...
    <title><?php echo "Class ".$_SESSION['class_teacher']?></title>
    ...
    <div class="wrapper">
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
           ...
           <button class="btn float-right btn-sm btn-default" data-toggle="modal" data-target="#attendanceRecord">Attendance Record</button><br><br>
                <h3 align="center"><?php echo "CLASS ".$_SESSION['class_teacher']?></h3><br>
                <h5><?php echo "ATTENDED BY : <b>".$_SESSION['first']." ".$_SESSION['last']."</b>"?></h5>
                ...
                <form method="POST">
                 <table class="table table-striped">
                    ...
                    <tbody >
                <?php ($staffdatabase->studentAttendanceByClass($_SESSION['class_teacher'],$_SESSION['post_assigned']))?>
                </tbody>
                  </table>                    
                 <?php echo (!$staffdatabase->markedToday(date("Y-m-d",time()),$_SESSION['class_teacher']))? '<button class="btn btn-warning float-right" type="submit" id="attendance_submit" name="attendance_submit">SUBMIT</button>': '<span class="float-right"><b>MARKED TODAY</b></span>'; ?>
                 </form>
              </div>
          </div> 
</div>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content ">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">ATTENDANCE RECORD</h5>
      ...
      <table class="table table-bordered attend">
         ...
         <tbody>
        <?php $staffdatabase->myClassAttendanceRecord($_SESSION['class_teacher'],$_SESSION['post_assigned']);?>
        </tbody>
        </table>
      </div>
...
<?php
    if(isset($_POST["attendance_submit"])){
        ...
        $classTeacherName=$_SESSION['first']." ".$_SESSION['last'];
        ...
        for($count=0; $count < count($student_id); $count++)
        {
            $data=array(
                ...
                ':attended_by'=>$classTeacherName
            );
           $staffdatabase->setAttendence($data[':student_id'],$data[':attend_status'],$data[':attendence_date'],$data[':attended_by'],$_SESSION['class_teacher']);
        }
    }
?>
